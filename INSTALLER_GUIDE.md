...
## Overview
This guide explains how to create a professional Windows installer for BlockTracker using Inno Setup.
...
.\build_windows.ps1
```
This creates `dist\BlockTracker\BlockTracker.exe` and all dependencies.
...
2. Click **File → Open**
3. Navigate to and open `BlockTracker.iss`
4. Click **Build → Compile** (or press F9)
5. The installer will be created in `packaging\output\BlockTracker-Setup-0.1.0.exe`
...
- **File**: `packaging\output\BlockTracker-Setup-0.1.0.exe`
- **Size**: Approximately 30-50 MB (contains full application)

Users simply:
1. Download `BlockTracker-Setup-0.1.0.exe`
...
1. Update version in `BlockTracker.iss` (line: `AppVersion=0.1.0`)
...
- Check that `dist\BlockTracker\BlockTracker.exe` exists
